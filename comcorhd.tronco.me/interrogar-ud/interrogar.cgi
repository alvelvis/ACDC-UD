...
<p>'''+str(i+1)+''' / '''+ocorrencias+'''</p>'''
		...
		html1 += '''<form action="../inquerito.py?conllu='''+ud+'''" target="_blank" method="POST" id="form_'''+str(i+1)+'''"><p><span id="text_'''+str(i+1)+'''">'''+ text.replace('/BOLD','</b>').replace('@BOLD','<b>').replace('@YELLOW/', '<font color="' + tabela['yellow'] + '">').replace('@PURPLE/', '<font color="' + tabela['purple'] + '">').replace('@BLUE/', '<font color="' + tabela['blue'] + '">').replace('@RED/', '<font color="' + tabela['red'] + '">').replace('@CYAN/', '<font color="' + tabela['cyan'] + '">').replace('/FONT', '</font>')+ '''</span><input type="hidden" name="textheader" value="''' + text.replace('/BOLD','').replace('@BOLD','').replace('@YELLOW/', '').replace('@PURPLE/', '').replace('@BLUE/', '').replace('@RED/', '').replace('@CYAN/', '').replace('/FONT', '') + '''"></p>
<p>'''
		...
		html1 += '''<p><a href="#">Voltar ao topo</a> &nbsp; <a style="cursor:pointer" onclick='inquerito("form_'''+str(i+1)+'''")'>Abrir inquérito</a></p></form></div>\n'''
