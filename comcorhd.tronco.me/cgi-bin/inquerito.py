...
if os.environ['REQUEST_METHOD'] == 'POST' and not 'action' in form.keys():
	...
	for i, sentence in enumerate(conlluzao):
		...
		if '# text = ' + form['textheader'].value + '\n' in sentence2 or '# sent_id = ' + form['textheader'].value + '\n' in sentence2:
			...
			for a, linha in enumerate(sentence2.splitlines()):
				if not '\t' in linha:
					html1 += '''<a style="cursor:pointer; color:white;" onclick="document.getElementById('valor').value = \'''' + linha.replace("'","\\'") + '''\'; document.getElementById('token').value = \'''' + str(a+1) + '''\'; document.getElementById('coluna').value = ''; $('valor').focus(); ">''' + linha + '</a>'
				else:
					for b, coluna in enumerate(linha.split('\t')):
						html1 += '''<a style="cursor:pointer; color:white;" onclick="document.getElementById('valor').value = \'''' + coluna.replace("'","\\'") + '''\'; document.getElementById('token').value = \'''' + str(a+1) + '''\'; document.getElementById('coluna').value = \'''' + str(b+1) + '''\'; $('valor').focus(); ">''' + coluna + '</a>&#9;'
				html1 += '<br>'

			html1 += '</pre><input type="hidden" name="textheader" value="' + form['textheader'].value + '"></label></form><br><a href="inquerito.py"><< Relatório de inquéritos</a>'
			achou = True
			break
	...
	try:
		...
		open('/interrogar-ud/inqueritos.txt', 'w').write(form['textheader'].value + '!@#' + antes + ' --> ' + depois + '!@#' + form['conllu'].value + '!@#' + data + '\n' + inqueritos)

		print('''<head>
				   <meta http-equiv="content-type" content="text/html; charset=UTF-8; width=device-width, initial-scale=1.0" name="viewport">
				 </head>
				 <body><script>window.alert("Inquérito concluído com sucesso!\\n\\nANTES:\\n''' + antes + '''\\n\\nDEPOIS:\\n''' + depois + '''"); window.close();</script></body>''')
